...
// Bag randomization system for better piece distribution
class TetrominoBag {
  ...
  private refillBag(): void {
    const allPieces: TetrominoType[] = ["I", "J", "L", "O", "S", "T", "Z"];
    this.bag = this.shuffleArray(allPieces);
  }

  getNextPiece(): TetrominoShape {
    if (this.bag.length === 0) {
      this.refillBag();
    }
    const pieceType = this.bag.pop()!;
    return TETROMINOS[pieceType];
  }
}
...
const randomTetromino = (): TetrominoShape => {
  return tetrominoBag.getNextPiece();
};
...
export default function Tetris() {
  ...
  const checkCollision = useCallback(
    (x: number, y: number, shape: number[][]): boolean => {
  ...
  const spawnNewPiece = useCallback((): void => {
  ...
  useEffect(() => {
    if (!gameOver && dropInterval.current) {
  ...
  const renderCell = (x: number, y: number): TetrominoColor | 0 => {
    if (
      currentPiece &&
